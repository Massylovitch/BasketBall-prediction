...
def process_for_prediction(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["MATCHUP"] = (
        df["VISITOR_TEAM_ID"].map(NBA_TEAMS_NAMES)
        + " @ "
        + df["HOME_TEAM_ID"].map(NBA_TEAMS_NAMES)
    )
...
def get_model(project, model_name, evaluation_metric, sort_metrics_by):
    ...
    model = mr.get_best_model(model_name, evaluation_metric, sort_metrics_by)
...
st.write(
    " - For the 2022-23 regular season (not playoffs), the current model would have an accuracy of 0.615."
)
st.write(" - One of the best publicly available models achieved an accuracy of 0.656.")
st.write(
    " - A simple baseline model of 'home team always wins' would have an accuracy of 0.58."
)
st.write("")
st.write(
...
if no_games == False:
    ...
    df_todays_matches["GAME_DATE_EST"] = df_todays_matches["GAME_DATE_EST"].dt.strftime(
        "%Y-%m-%d"
    )
...
if no_games == False:
    ...
    df_todays_matches = df_todays_matches.reset_index(drop=True)
    st.dataframe(
        df_todays_matches[["GAME_DATE_EST", "MATCHUP", "HOME_TEAM_WIN_PROBABILITY"]]
    )
...
df_current_season["HOME_TEAM_WIN_PROBABILITY_INT"] = (
    df_current_season["HOME_TEAM_WIN_PROBABILITY"].round().astype(int)
)
df_current_season["CORRECT_PREDICTION"] = (
    df_current_season["HOME_TEAM_WIN_PROBABILITY_INT"] == df_current_season["HOME_WINS"]
)
...
# format date
df_current_season["GAME_DATE_EST"] = df_current_season["GAME_DATE_EST"].dt.strftime(
    "%Y-%m-%d"
)

# clean up display
df_current_season = df_current_season.rename(
    columns={
...
st.dataframe(
    df_current_season[["GAME_DATE", "MATCHUP", "HOME_WIN_PROB", "HOME_WINS", "CORRECT"]]
)

# Show accuracy
st.write(
    "Accuracy: " + str(df_current_season["CORRECT"].sum() / df_current_season.shape[0])
)
